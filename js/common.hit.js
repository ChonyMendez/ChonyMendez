...
WebPlatform.collectPageStats = () => {
  ...
  const isPreview =
    WebPlatform.getUrlQueryParameter(window.location.href, "preview") !== "";
  if (
    !WebPlatform.areSystemAnalyticsEnabled() ||
    window.location.pathname.indexOf("/_preview") === 0 ||
    isPreview ||
    WebPlatform.pageForbidden === true
  ) {
    return;
  }
  ...
  let data = {
    ...
    funnelVar:
      WebPlatform.Funnels && WebPlatform.Funnels.variant
        ? WebPlatform.Funnels.variant
        : "",
    _r: new Date().getTime(),
  };
...
WebPlatform.PageStatsService = (() => {
  ...
  const set = (key, sessionDetails) => {
    ...
    if (
      ...
      WebPlatform.Funnels.steps.length
    ) {
      let key = "wpvp" + WebPlatform.Funnels.steps[0].funnelId;
      let value =
        sessionDetails && sessionDetails.funnelProgress
          ? sessionDetails.funnelProgress[key]
          : "";
      if (value) {
